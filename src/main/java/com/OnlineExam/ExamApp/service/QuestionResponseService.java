...
public class QuestionResponseService {
    ...
    public ResponseEntity<?> submitAllQuestion(BulkSubmitRequest bulkSubmitRequest) {
        ...
        if (users.isAttempted()) {
            return new ResponseEntity<>("user already submitted the answer", HttpStatus.OK);
        } else {
            users.setAttempted(true);
            ...
            users.setScore(countOfCorrectAnswer-0.25*countOfWrongAnswer);
